 ...
 * Theme setup
 */
function miles_local_music_shop_5c562150_setup() {
...
add_action( 'after_setup_theme', 'miles_local_music_shop_5c562150_setup' );
 ...
 */
function miles_local_music_shop_5c562150_enqueue_frontend_styles() {
    ...
    wp_enqueue_style(
        'miles-local-music-shop-5c562150-styles',
        get_template_directory_uri() . '/assets/css/styles.css',
        array(),
        $styles_version
    );
}
add_action( 'wp_enqueue_scripts', 'miles_local_music_shop_5c562150_enqueue_frontend_styles' );

/**
 * Enqueue theme scripts for frontend only
 */
function miles_local_music_shop_5c562150_enqueue_scripts() {
    ...
    if ( file_exists( $script_path ) ) {
        wp_enqueue_script(
            'miles-local-music-shop-5c562150-script',
...
add_action( 'wp_enqueue_scripts', 'miles_local_music_shop_5c562150_enqueue_scripts' );

/**
 * Remove title support from pages for cleaner editing experience.
 * The homepage content provides its own visual header.
 */
function miles_local_music_shop_5c562150_customize_pages() {
    remove_post_type_support( 'page', 'title' );
}
add_action( 'init', 'miles_local_music_shop_5c562150_customize_pages' );

/**
 * Add JS class to html element for animation system.
 * Outputs early in head to prevent FOUC on animated elements.
 */
function miles_local_music_shop_5c562150_add_js_class() {
    echo '<script>document.documentElement.classList.add("js");</script>';
}
add_action( 'wp_head', 'miles_local_music_shop_5c562150_add_js_class', 1 );
